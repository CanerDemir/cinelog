...
void main() {
  group('SplashScreen Unit Tests - Core Functionality', () {
    group('Animation Controller Initialization and Disposal', () {
      test('should initialize SplashConfig with default values correctly', () {
      ...
      test('should handle custom configuration parameters', () {
        ...
        expect(
            config.logoAnimationDuration, const Duration(milliseconds: 1000));
    ...
    group('Timer Functionality Logic', () {
      test('should validate timer duration calculations', () {
        const config =
            SplashConfig(displayDuration: Duration(milliseconds: 3000));
      ...
      test('should validate animation sequencing logic', () {
        ...
        expect(
            navigationTime,
            greaterThan(
                logoStartTime + config.logoAnimationDuration.inMilliseconds));
      });
    ...
    group('Responsive Sizing Calculations', () {
      ...
      test('should handle landscape orientation constraints', () {
        ...
        double finalLogoSize = maxHeightBasedSize < logoSizeWidth
            ? maxHeightBasedSize.clamp(80.0, 150.0)
            : logoSizeWidth;
    ...
    group('Configuration Handling Logic', () {
    ...
    group('Error Handling Logic', () {
      ...
      test('should validate configuration consistency', () {
        ...
        expect(config.logoAnimationDuration.inMilliseconds,
            greaterThanOrEqualTo(0));
        expect(config.textAnimationDuration.inMilliseconds,
            greaterThanOrEqualTo(0));
        expect(
            config.transitionDuration.inMilliseconds, greaterThanOrEqualTo(0));
        expect(
            config.textAnimationDelay.inMilliseconds, greaterThanOrEqualTo(0));
    ...
    group('Navigation Triggering Logic', () {
      test('should validate navigation timing calculations', () {
        const config = SplashConfig();

        // Navigation should happen after all animations have had time to complete
        int logoCompleteTime = config.logoAnimationDuration.inMilliseconds;
        int textStartTime = config.textAnimationDelay.inMilliseconds;
        int textCompleteTime =
            textStartTime + config.textAnimationDuration.inMilliseconds;
        int navigationTime = config.displayDuration.inMilliseconds;

        expect(navigationTime, greaterThan(logoCompleteTime));
        expect(navigationTime, greaterThan(textCompleteTime));
      });

      test('should validate skip splash functionality', () {
        const skipConfig = SplashConfig(skipInDebug: true);
        const normalConfig = SplashConfig(skipInDebug: false);

        // Skip logic should work correctly
        expect(skipConfig.shouldSkipSplash(), true); // In debug mode
        expect(normalConfig.shouldSkipSplash(), false);
      });
    });
  });
}
